...
def save_stage_pie(preds, save_path="static/percent_pie.png"):
    ...
    for stage in ['N1', 'N2', 'N3', 'REM']:
        stage_idx = stage_names.index(stage)  # 1~4
        count = counts.get(stage_idx, 0)
        if count > 0:
            labels.append(stage)
            sizes.append(count)
...
def result():
    ...
    status_info = {}
    for stage in ideal_ratio:
        actual = actual_ratio[stage]
        ideal = ideal_ratio[stage]
        if actual < ideal * 0.95:
            status = "부족"
        elif actual > ideal * 1.05:
            status = "여유"
        else:
            status = "적정"
        status_info[stage] = status

    # ✅ 각 단계별 bar 표시 범위 계산 (±5%)
    bar_ranges = {}
    for stage, ideal in ideal_ratio.items():
        start = max((ideal - 0.05) * 100, 0)
        end = min((ideal + 0.05) * 100, 100)
        bar_ranges[stage] = (round(start, 1), round(end, 1))

    sleep_score = int(100 * (actual_ratio[2] * 0.5 + actual_ratio[3] * 0.3 + actual_ratio[4] * 0.2))

    report = Report(
        user_id=current_user.id,
        preds_json=json.dumps(preds),
        sleep_score=sleep_score,
        major_stage=major_stage_label
    )
    ...
    return render_template("result.html",
                           ...
                           gap_n3=gap_info[3],
                           gap_rem=gap_info[4],
                           status_n1=status_info[1],
                           status_n2=status_info[2],
                           status_n3=status_info[3],
                           status_rem=status_info[4],
                           character_img=character_img,
                           stage_description=stage_description,
                           bar_n1_start=bar_ranges[1][0], bar_n1_end=bar_ranges[1][1],
                           bar_n2_start=bar_ranges[2][0], bar_n2_end=bar_ranges[2][1],
                           bar_n3_start=bar_ranges[3][0], bar_n3_end=bar_ranges[3][1],
                           bar_rem_start=bar_ranges[4][0], bar_rem_end=bar_ranges[4][1])
...
def report_detail(report_id):
    ...
    # ✅ 적정 구간 범위 ±5%로 계산 (퍼센트 단위)
    bar_range_info = {
        1: (ideal_ratio[1] * 100 * 0.95, ideal_ratio[1] * 100 * 1.05),  # N1
        2: (ideal_ratio[2] * 100 * 0.95, ideal_ratio[2] * 100 * 1.05),  # N2
        3: (ideal_ratio[3] * 100 * 0.95, ideal_ratio[3] * 100 * 1.05),  # N3
        4: (ideal_ratio[4] * 100 * 0.95, ideal_ratio[4] * 100 * 1.05),  # REM
    }
    ...
    return render_template("result.html",
                           ...
                           status_n3=status_info[3],
                           status_rem=status_info[4],
                           bar_n1_start=bar_range_info[1][0], bar_n1_end=bar_range_info[1][1],
                           bar_n2_start=bar_range_info[2][0], bar_n2_end=bar_range_info[2][1],
                           bar_n3_start=bar_range_info[3][0], bar_n3_end=bar_range_info[3][1],
                           bar_rem_start=bar_range_info[4][0], bar_rem_end=bar_range_info[4][1],
                           character_img=character_img,
                           stage_description=stage_description)
